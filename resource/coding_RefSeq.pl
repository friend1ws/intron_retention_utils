    ...
    if ($F[1] =~ /^NM/) {
        ...
        for (my $i = 0; $i <= $#starts; $i++) {
            ...
            if (min($ends[$i], $F[6]) - $starts[$i] > 0) {
                ...
                if ($F[3] eq "+") {
                    print OUT_5PUTR $F[2] . "\t" . $starts[$i] . "\t" . min($ends[$i], $F[6]) . "\n";
                } else {
                    print OUT_3PUTR $F[2] . "\t" . $starts[$i] . "\t" . min($ends[$i], $F[6]) . "\n";
                }

            }

            if (min($ends[$i], $F[7]) - max($starts[$i], $F[6]) > 0) {
                print OUT_EXON $F[2] . "\t" . max($starts[$i], $F[6]) . "\t" . min($ends[$i], $F[7]) . "\n";
            }

            if ($ends[$i] - max($F[7], $starts[$i]) > 0) {

                if ($F[3] eq "+") { 
                    print OUT_3PUTR $F[2] . "\t" . max($F[7], $starts[$i]) . "\t" . $ends[$i] . "\n";
                } else {
                    print OUT_5PUTR $F[2] . "\t" . max($F[7], $starts[$i]) . "\t" . $ends[$i] . "\n";
                }
        ...
        for (my $i = 1; $i <= $#starts; $i++) {
            print OUT_INTRON $F[2] . "\t" . $ends[$i - 1] . "\t" . $starts[$i] . "\n";
        }
    }
}
close(IN);
